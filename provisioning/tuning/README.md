...
`install.sh` copies it to `/var/lib/yondergate/tuning/imx519-af.json`. Point a camera at it
in **Setup › Cameras › Tuning file** and pick a focus mode. Verified on a Pi 4B (Bookworm,
kernel 6.12) with an Arducam 16 MP IMX519 — on the YonderRC vehicle this file came from,
not on a gateway, but it is the same file and the same libcamera: without it
...
that licence covers this derivative too, not the project's own LICENSE.
