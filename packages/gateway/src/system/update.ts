...
export interface UpdateImpact {
  ...
  /** The gateway service itself changed → a restart is enough. */
  service: boolean;
}
...
export function classifyChanges(files: string[]): UpdateImpact {
  ...
  return {
    ...
    service: f.some((p) => p.startsWith('packages/gateway/') || p.startsWith('packages/protocol/')),
  };
}
...
/**
 * The commands to run, in order. Dependencies first, restart last — the service
...
export function explainGitFailure(out: string): { cause: string; fix: string; selfFixable: boolean } {
  ...
  if (/authentication failed|permission denied \(publickey\)|terminal prompts disabled/i.test(log)) {
    return {
      ...
      fix: 'Point the checkout at the public HTTPS URL: `sudo git -C /opt/yondergate remote set-url origin https://github.com/TechnikWeber/YonderGate.git`.',
      selfFixable: false,
    };
  }
  if (/couldn't find remote ref|unknown revision|ambiguous argument/i.test(log)) {
    return {
      ...
      fix: 'Check `git -C /opt/yondergate status` over SSH.',
      selfFixable: false,
    };
  }
  return {
