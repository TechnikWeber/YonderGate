...
export const TUNING_DIR = '/var/lib/yondergate/tuning';
...
const BEGIN = '# --- YonderGate camera module (managed by the setup page) ---';
const END = '# --- end YonderGate camera module ---';
...
export function applyCameraModule(text: string, overlay: string | null): string {
  const body = stripManagedBlock(text)
    ...
    .map((raw) => {
      ...
      if (/^camera_auto_detect\s*=/.test(line)) return `# ${raw}  # (replaced by YonderGate)`;
      const dt = /^dtoverlay\s*=\s*(.+)$/.exec(line);
      if (dt && CAMERA_OVERLAYS.includes(overlayBaseName(dt[1]))) {
        return `# ${raw}  # (replaced by YonderGate)`;
      }
      return raw;
    })
    .join('\n');
