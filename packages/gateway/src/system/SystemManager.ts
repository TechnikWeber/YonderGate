import type { LinkSignal } from '@yondergate/protocol';
...
/**
 * SystemManager is the seam between YonderGate and the Pi's OS-level networking:
...
export interface SystemManager {
  ...
  /**
   * Apply the update. `hardwareDeps` are the native driver modules this box has
   * recorded: `npm install --omit=optional` prunes them, so they are reinstalled as
   * part of the update instead of leaving a configured gateway running as a simulator.
   */
