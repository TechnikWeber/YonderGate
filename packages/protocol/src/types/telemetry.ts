...
export interface CameraCfg {
   ...
   * AK7375 actuator is bound. See docs/HARDWARE.md.
   */
  tuningFile?: string;
}
...
/**
 * The uplink the gateway is currently reachable over. Shown on the status page, so
 * you can tell at a glance whether the site is on LTE, WiFi or a cable — and how
 * good that link is before you rely on it.
 */
export interface LinkSignal {
  kind: 'lte' | 'wifi' | 'ethernet' | 'none';
  /** 0..100 quality, or null if unknown. */
  quality: number | null;
  /** Short label, e.g. "LTE 72%" or "WiFi −58 dBm". */
  label: string;
}
