...
RUN npm install --omit=dev --workspace @yondergate/gateway || npm install --workspace @yondergate/gateway

COPY packages/protocol packages/protocol
COPY packages/vehicle packages/vehicle

ENV YGW_HOST=0.0.0.0
ENV YGW_PORT=8080
CMD ["npm", "run", "start", "-w", "@yondergate/gateway"]
